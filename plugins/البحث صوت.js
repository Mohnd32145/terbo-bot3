    ...
    try {
      ...
      const { title, videoUrl, thumbnail } = playmp3;
           ...
           await conn.sendMessage(m.chat, { react: { text: '✔️', key: m.key } });
             ...
             conn.sendMessage(m.chat, {document: {url: videoUrl}, mimetype: 'audio/mpeg', fileName: title + '.mp3'}, {quoted: m});
             
          } catch {
          
            await conn.reply(m.chat, '*❲ ❗ ❳ حدث خطأ عند جلب الصوت.*', m);
          }
        
      
      
        } catch {
      await conn.sendMessage(m.chat, { text: `*❲ ❗ ❳ حدث خطأ عند البحث عن الصوت .*\nيرجي ادخال نص صحيح أو رابط مثال :\n> ➤  ${usedPrefix + command} القرآن الكريم\n> ➤  ${usedPrefix + command} https://youtu.be/JLWRZ8eWyZo?si=EmeS9fJvS_OkDk7p` }, { quoted: m });
      await conn.sendMessage(m.chat, { react: { text: '❌', key: m.key } });
    }
